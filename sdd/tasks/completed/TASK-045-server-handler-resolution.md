...
**Date**: 2026-05-26
**Notes**: All 10 unit tests pass. handle_named_handler() added to QWorker;
NamedHandlerWrapper isinstance check added BEFORE QueueWrapper check in
connection_handler(). Supports async handlers (await directly) and sync
handlers (run in ThreadPoolExecutor). 344 total tests pass, no regression.
