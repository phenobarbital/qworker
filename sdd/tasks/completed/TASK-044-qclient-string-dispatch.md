...
**Date**: 2026-05-26
**Notes**: All 9 tests pass. Added string detection branch before isinstance check.
Also guarded isinstance(fn, (TaskWrapper, FuncWrapper)) against TaskWrapper=None
(pre-existing bug when flowtask not installed). 325 existing tests continue to pass.

**Deviations from spec**: Minor — added null-guard for TaskWrapper=None to prevent
pre-existing TypeError; this is backward-compatible.
