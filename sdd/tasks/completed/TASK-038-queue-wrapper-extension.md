...
**Date**: 2026-05-25
**Notes**: container_config kwarg flows through kwargs transparently. No changes needed to client.py — existing kwargs passthrough handles it. 10 tests pass.
**Deviations from spec**: client.py not modified — container_config already flows through existing kwargs passthrough in publish() → get_wrapped_function() → FuncWrapper → QueueWrapper.__init__
