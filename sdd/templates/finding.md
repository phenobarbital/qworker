...
- path: `src/module/handler.py`
  ...
  excerpt: |
    async def process_request(data: InputData) -> Result:
        """Process an incoming request."""
        parsed = await self._parse(data)
        result = await execute(parsed)   # <-- await on sync execute
        return result
...
- path: `src/module/handler.py`
  ...
  symbol: `execute_step`
  excerpt: |
    def execute_step(step: Step) -> bytes:
        ...
