...
You are the Synthesis Agent for the SDD pipeline, Phase 3 of `/sdd-proposal`.
...
<findings>
  <finding id="F001" query_id="Q001" type="wiki_query|wiki_page|wiki_related|grep|read|git_log|glob|tree">
    ...
    <citations>
      - path: src/scheduler/dispatch.py
        lines: 47-89
        symbol: dispatch_task
...
```json
{
  ...
  "feat_id": "FEAT-156",
  "mode": "investigation",
  "summary_oneline": "Async refactor in Scheduler dispatch appears to await a sync helper, breaking task execution.",
  ...
  "localization": [
    {
      ...
      "symbol": "dispatch_task",
      "lines": "47-89",
      "role": "entrypoint that orchestrates task dispatching for the scheduler",
      "evidence": ["F001", "F003"]
    },
    {
      "path": "src/executor/runner.py",
      "symbol": "run",
      "lines": "12-58",
      "role": "shared synchronous executor used by Scheduler and other modules",
      "evidence": ["F004"]
    }
  ],
  ...
  "constraints": [
    {
      "statement": "Shared `executor.runner.run` is sync and called from multiple modules",
      "evidence": ["F004"],
      "implication": "any async refactor in scheduler must not break sync callers in legacy modules"
    },
    {
      "statement": "Scheduler tests are skipped in default CI runs",
      "evidence": ["F006"],
  ...
  "investigation": {
    "hypotheses": [
      {
        "rank": 1,
        "statement": "The async refactor in commit a3f4b2 awaits a sync function, causing a silent return of a coroutine that is never awaited downstream.",
        "supporting": ["F002", "F005"],
        "contradicting": [],
        "confidence": "medium",
        "reasoning": "Commit diff shows `await run(...)` added on a sync `run`. Test in F005 was skipped at the same commit.",
        "next_probe": "Run `pytest tests/scheduler/test_dispatch.py::test_dispatch_task -s` and check for 'coroutine never awaited' warning."
      }
    ]
  },
  ...
  "confidence_map": [
    {
      "id": "C1",
      "statement": "Task dispatch entrypoint lives at src/scheduler/dispatch.py:dispatch_task",
      "evidence": ["F001"],
      "confidence": "high",
    ...
    {
      "id": "C3",
      "statement": "Shared executor helper at src/executor/runner.py:run is sync",
      "evidence": ["F004"],
      "confidence": "high",
    ...
    {
      "id": "C4",
      "statement": "Test test_dispatch_task was skipped intentionally",
      "evidence": [],
      "confidence": "low",
  ...
  "unknowns": [
    {
      "id": "U1",
      "question": "Was tests/scheduler/test_dispatch.py::test_dispatch_task skipped intentionally or by accident in commit a3f4b2?",
      "blocks_claims": ["C4"],
      "plausible_answers": [
