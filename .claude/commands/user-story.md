...
/enrich-story --jira-ticket PROJ-1234                 # seed from the ticket AND inject back into it
/enrich-story --jira-ticket PROJ-1234 "make it per-tenant, not global"   # ticket = seed + target, text augments
...
Before proposing any default, search the workspace to align with real patterns. The codebase is the only authority. Read `CLAUDE.md` and `.agent/CONTEXT.md` at the start of every run to discover the project's package structure, core abstractions, naming conventions, and architectural invariants.

Use those files as search starting points — verify every claim against what you actually find in code. Discover the project's recurring patterns (registries, ABCs, mixins, async conventions, storage backends) from the source rather than assuming them.
...
- **curl env**: `JIRA_INSTANCE`, `JIRA_USERNAME`, `JIRA_API_TOKEN` (load from environment or `.env`).
  ```bash
...
✅ Enriched story ready: <title>
   Source: PROJ-1234  ·  Grounded against: <N> files
   Acceptance criteria: <N>   Open questions: <N>

   Jira: appended enriched block to PROJ-1234 (original description untouched)  [if --jira-ticket]
