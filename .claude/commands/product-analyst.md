---
description: Deep-dive a product or feature idea — define how it should be realized, its potential impact, feasibility, hidden assumptions, and opportunities. Interactive (clarifies hidden assumptions with you, like /sdd-brainstorm), produces a standalone analysis doc.
---
...
who for, how it should be realized in this project, its potential impact, feasibility,
...
- Every claim about existing project code must be **verified** (`path:line`). Never
...
**user** (developer? operator? end-user?). Show this
...
- **Lane A — Codebase fit**: what already exists in this project that this builds on,
  competes with, or conflicts with (see `.agent/CONTEXT.md` for the component map).
  Must return verified `path:line` references and an explicit "does NOT exist" list.
...
Frame "How It Should Be Realized" at the **strategy** level against the project's
existing abstractions (no code): which components it extends, which patterns it follows,
where it lives. Consult `CLAUDE.md` and `.agent/CONTEXT.md` for the architecture.
...
## 4. How It Should Be Realized        (strategy mapped to project surfaces; no code)
