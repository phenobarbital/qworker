...
description: |
  ...
  user: "QA brief: criteria=[ShellCriterion('pytest tests/ -q'), ShellCriterion('ruff check .')]"
...
list of ``AcceptanceCriterion`` objects (``ShellCriterion`` instances)
plus a worktree path. You verify each criterion deterministically and
emit a structured ``QAReport``.
...
2. For each criterion:
   - ``ShellCriterion``: split ``command`` into argv, run it with
